...
* :meth:`~mimiqcircuits.NoiseModel.add_operation_noise`
...
* :class:`~mimiqcircuits.OperationInstanceNoise`: Applies noise to all instances of a specific operation type (e.g., all ``CX`` gates, all measurements, all resets).
* :class:`~mimiqcircuits.SetOperationInstanceQubitNoise`: Applies noise to operations acting on qubits within a given set.
* :class:`~mimiqcircuits.ExactOperationInstanceQubitNoise`: Applies noise to operations acting on a specific sequence of qubits.
...
* :class:`~mimiqcircuits.CustomNoiseRule`: User-defined rule with custom matching and noise generation logic. Has the highest priority by default.
