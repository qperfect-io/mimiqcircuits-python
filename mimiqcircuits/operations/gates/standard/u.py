...
class GateUPhase(mcg.Gate):
    ...
    **Matrix representation:**
    ...
    Examples:
        ...
        [0.5*exp(I*gamma)*(1.0 + exp(I*theta)), (0.0 + 0.5*I)*exp(I*gamma + I*lambda)*(-1.0 + exp(I*theta))]
        [(-0.0 - 0.5*I)*exp(I*gamma + I*phi)*(-1.0 + exp(I*theta)), 0.5*exp(I*gamma + I*(lambda + phi))*(1.0 + exp(I*theta))]
        <BLANKLINE>
        >>> c = Circuit().push(GateUPhase(theta, phi, lmbda, gamma), 0)
        >>> c
        1-qubit circuit with 1 instructions:
        └── UPhase(theta, phi, lambda, gamma) @ q[0]
        <BLANKLINE>
    ...
    def _control(self, n):
        return control_one_defined(n, self, self.GateCU(self.theta, self.phi, self.lmbda, self.gamma))
...
class GateU(mcg.Gate):
    r"""Single qubit generic unitary gate.
    ...
    **Matrix representation:**
    ...
    Examples:
        ...
        [0.5 + 0.5*exp(I*theta), (0.0 + 0.5*I)*exp(I*lambda)*(-1.0 + exp(I*theta))]
        [(-0.0 - 0.5*I)*exp(I*phi)*(-1.0 + exp(I*theta)), 0.5*exp(I*(lambda + phi))*(1.0 + exp(I*theta))]
        <BLANKLINE>
        >>> c = Circuit().push(GateU(theta, phi, lmbda), 10)
        >>> c
        11-qubit circuit with 1 instructions:
        └── U(theta, phi, lambda) @ q[10]
        <BLANKLINE>
    ...
    def _control(self, n):
        return control_one_defined(n, self, self.GateCU(self.theta, self.phi, self.lmbda, 0))
